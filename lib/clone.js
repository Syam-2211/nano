...
const jadibot = async (NanoBotz, m, from) => {
  if (Object.keys(client).includes(from)) {
    return NanoBotz.sendMessage(from, {
    ...
    async function connectToWhatsApp() {
      ...
      if (usePairingCode && !client[from].user && !client[from].authState.creds.registered) {
        setTimeout(async () => {
          ...
          await NanoBotz.sendMessage(from, {text: txt }, {quoted: m})
        }, 2000)
      }
      ...
      client[from].ev.on('messages.upsert', async chatUpdate => {
        try {
          ...
          require('../Nano')(client[from], m, chatUpdate, store)
        } catch (err) {
          console.log(err)
        }
      })
      ...
      const jidDisplayName = (jid = '') => {
        ...
        try { return PhoneNumber('+' + clean).getNumber('international') } catch { return clean }
      }
...
async function stopjadibot(NanoBotz, m, from) {
  if (!Object.keys(client).includes(from)) {
    return NanoBotz.sendMessage(from, {
...
async function listjadibot(NanoBotz, m) {
